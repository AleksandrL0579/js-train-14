...
function addDays(date, days) {
  if (!(date instanceof Date)) {
    return {
      error: "Помилка: вхідне значення має бути об'єктом Date",
    };
  }

  if (typeof days !== "number") {
    return {
      error: "Помилка: кількість днів має бути числом",
    };
  }
  ...
  const addedMilliseconds = days * millisecondsInDay;
  const resultTimestamp = initialTimestamp + addedMilliseconds;
  const resultDate = new Date(resultTimestamp).toISOString();
...
function getDaysInMonth(date) {
  ...
  const lastDayOfCurrentMonth = new Date(nextMonthFirstDay - 86400000);
